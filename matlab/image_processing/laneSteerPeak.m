function error = laneSteerPeak(filtered_left_img,filtered_right_img, filtered_green_img)
%LANESTEERIMCROP Summary of this function goes here
    ...
    top_y = round(res_y / 10 * 2.5, 0);

    %adjustable peak settings
    pp = 3; %peak prominence, 5 is default
    ROI_width = 4; %ROI width, 10 is default
    stop_height_final = 120; %height of the final stop ROI from the bottom in pixels
    ...
    [left_top_sum, left_bot_sum, left_right_sum, left_stop_sum] = LeftSum(res_x, top_y, bot_y, res_y, filtered_left_img, ROI_width, vertical_height, stop_height_lanes);

    %making RIGHT LANE
    %crop top area of interest for analysis
    [right_top_sum, right_bot_sum, right_left_sum, right_stop_sum] = RightSum(res_x, top_y, bot_y, res_y, filtered_right_img, ROI_width, vertical_height, stop_height_lanes);

    %making GREEN STOP signal
    %final_stop_sum = FinalSum(res_x, res_y, filteredGreen, ROI_width, stop_height);
    final_stop_area = getFinalStopROI(filtered_green_img, res_x, res_y, stop_height_final);
...
function [left_top_sum, left_bot_sum, left_right_sum, left_stop_sum] = LeftSum(res_x, top_y, bot_y, res_y, leftImage, ROI_width, vertical_height, stop_height_lanes)
    ...
    left_right_area = imcrop(leftImage, [res_x / 2 - ROI_width / 2 - 1, vertical_height, 10, res_y * 0.75]);
...
function [right_top_sum, right_bot_sum, right_left_sum, right_stop_sum] = RightSum(res_x, top_y, bot_y, res_y, rightImage, ROI_width, vertical_height, stop_height_lanes)
    ...
    right_left_area = imcrop(rightImage, [res_x / 2 - ROI_width / 2 - 1, vertical_height, 10, res_y * 0.75]);
    %sum up all the white pixels with respect to the y-axis
