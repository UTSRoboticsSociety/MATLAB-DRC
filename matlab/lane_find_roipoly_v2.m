...
function [left_top_area, left_bot_area, left_right_area, left_stop_area] = getLeftROIs(leftImage, res_x, res_y, top_y, bot_y)
    ...
    left_top_mask = roipoly(leftImage, [1 res_x res_x 1], [top_y top_y top_y + 1 top_y + 1]);
    left_top_area = leftImage & left_top_mask;

    left_bot_mask = roipoly(leftImage, [1 res_x res_x 1], [bot_y bot_y bot_y + 1 bot_y + 1]);
    left_bot_area = leftImage & left_bot_mask;

    left_right_mask = roipoly(leftImage, [(res_x / 2) (res_x / 2 + 1) (res_x / 2 + 1) (res_x / 2)], [10 10 bot_y - 1 bot_y - 1]);
    left_right_area = leftImage & left_right_mask;

    left_stop_mask = roipoly(leftImage, [(res_x / 2) (res_x / 2 + 1) (res_x / 2 + 1) (res_x / 2)], [bot_y bot_y res_y res_y]);
    left_stop_area = leftImage & left_stop_mask;

end

function [right_top_area, right_bot_area, right_left_area, right_stop_area] = getRightROIs(rightImage, res_x, res_y, top_y, bot_y)

    right_top_mask = roipoly(rightImage, [1 res_x res_x 1], [top_y top_y top_y + 1 top_y + 1]);
    right_top_area = rightImage & right_top_mask;

    right_bot_mask = roipoly(rightImage, [1 res_x res_x 1], [bot_y bot_y bot_y + 1 bot_y + 1]);
    right_bot_area = rightImage & right_bot_mask;

    right_left_mask = roipoly(rightImage, [(res_x / 2) (res_x / 2 + 1) (res_x / 2 + 1) (res_x / 2)], [10 10 bot_y - 1 bot_y - 1]);
    right_left_area = rightImage & right_left_mask;

    right_stop_mask = roipoly(rightImage, [(res_x / 2) (res_x / 2 + 1) (res_x / 2 + 1) (res_x / 2)], [bot_y bot_y res_y res_y]);
    right_stop_area = rightImage & right_stop_mask;

end

function final_stop_area = getFinalStopROI(filteredGreen, res_x, res_y, stop_height)

    final_stop_mask = roipoly(filteredGreen, [(res_x / 2) (res_x / 2 + 1) (res_x / 2 + 1) (res_x / 2)], [(res_y - stop_height) (res_y - stop_height) res_y res_y]);
    final_stop_area = filteredGreen & final_stop_mask;
